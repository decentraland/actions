...
| `deployment-environments`  |          | —                                     | Environments to repoint, as a JSON array or comma list (e.g. '["zone"]'). Empty array '[]' stages the bytes in S3 without touching any KV. Defaults to zone only — staging and production are promoted deliberately from a job that declares a GitHub environment. Mutually exclusive with `deployment-environment`. |
